...
def _moc_hdrs_impl(ctx):
    ...
    all_headers = compilation_context.headers.to_list()
    moc_inputs = ctx.files.hdrs + all_headers

    substitution_pairs = dict()
    for h in ctx.files.hdrs:
        substitution_pairs["\"{old_name}\"".format(old_name = h.basename)] = "\"{new_name}\"".format(new_name = h.short_path)

    for hdr in ctx.files.hdrs:
        moc = ctx.actions.declare_file("moc_{name}".format(name = hdr.basename.removesuffix("." + hdr.extension)))
        ...
        ctx.actions.run(
            inputs = moc_inputs,  # make all headers visible to moc
            outputs = [moc, json],
        ...
        ctx.actions.expand_template(template = moc, output = cpp, substitutions = substitution_pairs)
...
def _moc_srcs_impl(ctx):
    ...
    #   and uses full paths relative to a project's root.
