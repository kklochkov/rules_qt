...
def _fetch(repository_ctx):
    ...
    for module, sha256 in repository_ctx.attr.modules.items():
        ...
        extracted = [f for f in folders if f.basename not in known_entries]
        known_entries = {f.basename: True for f in folders}

        if len(extracted) != 1:
            fail("Expected exactly one new directory after extracting '{module}', got {count}".format(
                module = module,
                count = len(extracted),
            ))
